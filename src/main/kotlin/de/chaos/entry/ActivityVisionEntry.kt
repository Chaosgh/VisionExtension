...
class ActivityVisionEntry(
    ...
    @Help(VisionEntryHelp.VISION_RADIUS)
    ...
    override fun create(
        ...
        if (ActivityVisionCreationGuard.isActive(this)) {
            return createVisionActivity(currentLocation)
        }

        return ActivityVisionCreationGuard.withActive(this) {
            val baseEntry = baseActivity.get()
            val base =
                if (ActivityVisionCreationGuard.isActive(baseEntry)) {
                    null
                } else {
                    baseEntry?.create(context, currentLocation)
                }
            wrapActivityWithVision(base, currentLocation, stopWhenLooking, resumeDelayTicks)
        }
    }
}

private object ActivityVisionCreationGuard {
    private val activeEntries =
        ThreadLocal.withInitial {
            ArrayDeque<String>()
        }

    fun isActive(entry: GenericEntityActivityEntry?): Boolean {
        return entry is ActivityVisionEntry && key(entry) in activeEntries.get()
    }

    fun <T> withActive(
        entry: ActivityVisionEntry,
        block: () -> T,
    ): T {
        val active = activeEntries.get()
        active.addLast(key(entry))
        return try {
            block()
        } finally {
            active.removeLast()
    ...
    private fun key(entry: ActivityVisionEntry): String {
        return entry.id.takeIf { it.isNotBlank() }
            ?: "identity:${System.identityHashCode(entry)}"
    }
}
